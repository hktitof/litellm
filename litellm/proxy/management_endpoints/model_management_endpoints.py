...
class ModelWrite:
    ...
    def registered_name(self) -> str | None:
        ...
        A create registers its ``model_name``. On an existing row the rename channel is judged by
        the same owner the update path uses (``_get_public_model_name``), so a dashboard save that
        echoes the current public name or the internal ``model_name_{team}_{uuid}`` key is not a
        rename, and neither a config-only update nor a delete registers anything.
        """
...
def member_may_write_auto_router(
    ...
    from a file on the proxy host, and stay on its team; an existing row must be one the member
    created; and a public name the write registers must not widen the team's model access.
    """
...
async def _update_team_model_in_db(
    ...
    if is_new_team_assignment:
        ...
        # The source lists the name the row carried there, which a rename in the same write changes.
        source_public_name: Final = (
            db_model.model_info.team_public_model_name if db_model.model_info else None
        ) or db_model.model_name
        if db_team_id is not None:
            await _release_team_model_name(
                team_id=db_team_id,
                public_model_name=source_public_name,
                moved_model_name=db_model.model_name,
                prisma_client=prisma_client,
            )
    else:
...
async def _release_team_model_name(
    team_id: str,
    public_model_name: str,
    moved_model_name: str,
    prisma_client: PrismaClient,
) -> None:
    """Drop a moved row's public name from its former team unless a sibling there still backs it.

    Without this the source team keeps a grant to a name nothing of theirs serves, which a
    later deployment of that name anywhere on the proxy would silently satisfy.
    """
    siblings: Final = await _get_team_deployments(team_id, prisma_client)
    still_backed: Final = any(
        sibling.model_name != moved_model_name
        and (model_info_as_mapping(sibling.model_info) or {}).get("team_public_model_name") == public_model_name
        for sibling in siblings
    )
    if still_backed:
        return
    await _unregister_team_models(team_id, (public_model_name,), prisma_client)
