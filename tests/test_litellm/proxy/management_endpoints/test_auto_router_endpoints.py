...
async def test_dry_runs_admit_a_member_whose_team_grants_auto_router_management(monkeypatch: pytest.MonkeyPatch):
    ...
    assert verdict.valid is True
    # The rehearsed create names no existing row and no served model group, so the same member who
    # may not edit a teammate's router still gets the dry run for a router of their own.
